/*
@author Ctrl -> Parker
@version 20190917 -> 2020-05-08
@aim 对xlsx-style方法进行二次封装 方便调用以导出带样式Excel
@aim 对原作者 Ctrl 的开源包进行 二次维护和BUG修正
...
XSU=({
	...
	setColWidthAll: function(workBook, sheetName, width) {
		var colsNum = this.getMaxColNum(workBook,sheetName);//当前最大列
		...
		this.setColWidth(workBook, sheetName, widths);
		return workBook;
	},
	//设置每列列宽,单位px cols= [{wpx: 45}, {wpx: 165}, {wpx: 45}, {wpx: 45}]
	...
	*/
	...
	setTitleStylesDefault: function(workBook, sheetName) {
		for (var cell in workBook.Sheets[sheetName]) {
			var num= cell.replace(/[^0-9]/ig,"");
			if("1" === num){
				//this.setFillFgColorRGB(workBook, sheetName, cell, '#FFFF00');
				this.setAlignmentHorizontal(workBook, sheetName, cell, 'center');
				this.setFontBold(workBook, sheetName, cell, true);
				this.setFontSize(workBook, sheetName, cell, '16');
			}	
		}
	},
	
	//设置第二行标题默认样式
	setSecondRowStylesDefault: function(workBook, sheetName) {
		for (var cell in workBook.Sheets[sheetName]) {
			var num= cell.replace(/[^0-9]/ig,"");
			if("2" === num){
				this.setFillFgColorRGB(workBook, sheetName, cell, '808080');
				this.setFontColorRGB(workBook,sheetName,cell,'FFFFFF');
				this.setAlignmentHorizontal(workBook, sheetName, cell, 'center');
				this.setFontBold(workBook, sheetName, cell, true);
			}	
		}
	...
	//当前表格最大列 A起步 return string
	getMaxCol: function(workBook,sheetName) {
		var length = 0;
		var rows1 = [];
		for (var ever in workBook.Sheets[sheetName]) {
			var num= ever.replace(/[^0-9]/ig,"");
			if("1" === num){
				rows1.push(ever);
			}	
		}
		return rows1[rows1.length-1];
	},
